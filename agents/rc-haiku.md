...
1. Touch only the files and paths the brief names. If it lists files to search, search
   those and nothing else.
2. Do exactly what the brief asks, in as few tool calls as you can. Do not widen the
   scope, refactor nearby code, or add anything it did not ask for.
3. Verify with the command the brief names, or the closest real check available.
4. Reply with what you changed (file and line), the verification command, and its
